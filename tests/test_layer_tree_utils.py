...
def test_refresh_layer_tree_view_uses_recursive_emit_when_available():
    iface = MagicMock()
    view = MagicMock()
    model = MagicMock()
    view.model.return_value = model
    ...
    project = MagicMock()
    root = MagicMock()
    root.checkedLayers.return_value = ["layer-a"]
    project.layerTreeRoot.return_value = root
    QgsProject.instance.return_value = project

    assert refresh_layer_tree_view(iface) is True
    model.recursivelyEmitDataChanged.assert_called_once_with()
    model.layoutChanged.emit.assert_called_once()
