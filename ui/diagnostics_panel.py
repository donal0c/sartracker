        ...
        try:
            ...
            if 'sartracker' in detail_plugins:
                ...
                if hasattr(detail_plugin, 'get_plugin_status'):
                    ...
                    if cache_stats:
                        # Generic provider file-cache stats (if exposed)
                        cache_entries = cache_stats.get('entries')
                        cache_max = cache_stats.get('max_entries')
                        if cache_entries is not None:
                            provider_details_lines.append(f"File Cache: {cache_entries}/{cache_max} files")
                        cache_positions = cache_stats.get('total_positions')
                        if cache_positions is not None:
                            provider_details_lines.append(f"Cached Positions: {cache_positions}")
                        cache_memory = cache_stats.get('memory_kb')
                        if cache_memory is not None:
                            provider_details_lines.append(f"Est. Memory: {cache_memory} KB")
